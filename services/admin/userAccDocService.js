...
const user_acc_doc_mst_list = async (req, res, next) => {
    ...
    try {
        ...
        const _query0 = `SELECT count(1) AS total_record FROM user_acc_doc_mast WHERE is_deleted = false AND LOWER(doc_name) LIKE LOWER(:search_text)`;
        ...
        FROM user_acc_doc_mast m WHERE m.is_deleted = false AND LOWER(m.doc_name) LIKE LOWER(:search_text) LIMIT :page_size OFFSET ((:page_no - 1) * :page_size)`;
        ...
        for (let i = 0; row1 && i < row1.length; i++) {
            var map_entity = [];
            const _query2 = `SELECT m.entity_id, e.entity_name, m.is_required FROM user_acc_doc_mapp m INNER JOIN entity_type e ON m.entity_id = e.entity_id WHERE m.document_id = ?`;
...
const user_acc_doc_mst_get = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query1 = `SELECT document_id, doc_name, doc_lng_key, file_type_allowed, file_max_size, is_enabled, added_date, modify_date FROM user_acc_doc_mast WHERE document_id = ? AND is_deleted = false`;
...
const user_acc_doc_mst_new = async (req, res, next) => {
    const { doc_name, doc_lng_key, file_type_allowed, file_max_size, map_entity, sort_order } = req.body;
    try {
        ...
        const _query1 = `SELECT document_id FROM user_acc_doc_mast WHERE LOWER(doc_name) = LOWER(?) AND is_deleted = false`;
        ...
        const _query2 = `INSERT INTO user_acc_doc_mast(doc_name, doc_lng_key, file_type_allowed, file_max_size, is_enabled, is_deleted, added_by, added_date, sort_order)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING "document_id"`;
        const _replacements2 = [doc_name.trim(), doc_lng_key.trim(), _file_type_allowed.join(','), _file_max_size, true, false, req.token_data.account_id, new Date(), _sort_order];
        ...
        if (document_id > 0) {
            for (let k = 0; _map_entity && k < _map_entity.length; k++) {
                const _query3 = `INSERT INTO user_acc_doc_mapp(document_id, entity_id, is_required) VALUES (?, ?, ?)`;
                const _replacements3 = [document_id, _map_entity[k].entity_id, _map_entity[k].is_required];
...
const user_acc_doc_mst_update = async (req, res, next) => {
    const { document_id, doc_name, doc_lng_key, file_type_allowed, file_max_size, map_entity, sort_order } = req.body;
    try {
        if (!doc_name || doc_name.length <= 0 || doc_name.trim().length <= 0) {
            return res.status(200).json(success(false, res.statusCode, "Please enter document name.", null));
        }
        ...
        const _query0 = `SELECT document_id FROM user_acc_doc_mast WHERE document_id = ? AND is_deleted = false`;
        ...
        const _query1 = `SELECT document_id FROM user_acc_doc_mast WHERE document_id <> ? AND LOWER(doc_name) = LOWER(?) AND is_deleted = false`;
        ...
        const _query2 = `UPDATE user_acc_doc_mast SET sort_order = ?, doc_name = ?, doc_lng_key = ?, file_type_allowed = ?, file_max_size = ?, modify_by = ?, modify_date = ? WHERE document_id = ?`;
        ...
        if (i > 0) {
            for (let k = 0; _map_entity && k < _map_entity.length; k++) {
                const _query3 = `INSERT INTO user_acc_doc_mapp(document_id, entity_id, is_required) 
                SELECT :document_id, :entity_id, :is_required WHERE NOT EXISTS (
                    SELECT entity_id FROM user_acc_doc_mapp WHERE document_id = :document_id AND entity_id = :entity_id
                )`;
                await db.sequelize.query(_query3, {
                    replacements: {
                        document_id: _document_id, entity_id: _map_entity[k].entity_id, is_required: _map_entity[k].is_required
                    }, type: QueryTypes.INSERT
                });

                const _query5 = `UPDATE user_acc_doc_mapp SET is_required = ? WHERE document_id = ? AND entity_id = ?`;
                await db.sequelize.query(_query5, { replacements: [_map_entity[k].is_required, _document_id, _map_entity[k].entity_id], type: QueryTypes.UPDATE });
            }
            var temp = [];
            for (let k = 0; _map_entity && k < _map_entity.length; k++) {
                temp.push(_map_entity[k].entity_id);
            }
            const _query4 = `DELETE FROM user_acc_doc_mapp WHERE document_id = ? AND entity_id NOT IN (?)`;
...
const user_acc_doc_mst_toggle = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query1 = `SELECT document_id, doc_name, is_enabled FROM user_acc_doc_mast WHERE document_id = ? AND is_deleted = false`;
        const row1 = await db.sequelize.query(_query1, { replacements: [_document_id], type: QueryTypes.SELECT });
        if (!row1 || row1.length <= 0) {
            return res.status(200).json(success(false, apiStatus.RELOAD_PAGE,"Document details not found.", null));
        }

        const _query2 = `UPDATE user_acc_doc_mast SET is_enabled = CASE WHEN is_enabled = true THEN false ELSE true END, modify_date = ?, modify_by = ? WHERE document_id = ?`;
...
const user_acc_doc_mst_delete = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query3 = `SELECT document_id FROM user_acc_doc_mast WHERE document_id = ? AND is_deleted = false`;
        const row3 = await db.sequelize.query(_query3, { replacements: [_document_id], type: QueryTypes.SELECT });
        if (!row3 || row3.length <= 0) {
            return res.status(200).json(success(false, res.statusCode, "Document details not found.", null));
        }
        const _query2 = `UPDATE user_acc_doc_mast SET is_deleted = true, deleted_date = ?, deleted_by = ? WHERE document_id = ?`;
        const _replacements2 = [new Date(), req.token_data.account_id, _document_id];
        const [, i] = await db.sequelize.query(_query2, { replacements: _replacements2, type: QueryTypes.UPDATE });
        if (i > 0) {
...
module.exports = {
    user_acc_doc_mst_list,
    user_acc_doc_mst_get,
    user_acc_doc_mst_new,
    user_acc_doc_mst_update,
    user_acc_doc_mst_toggle,
    user_acc_doc_mst_delete,
};