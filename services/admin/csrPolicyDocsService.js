...
const csr_policy_docs_list = async (req, res, next) => {
    ...
    try {
        ...
        const _query0 = `SELECT count(1) AS total_record FROM csr_policy_doc_mast WHERE is_deleted = false AND LOWER(doc_name) LIKE LOWER(:search_text)`;
        ...
        FROM csr_policy_doc_mast m WHERE m.is_deleted = false AND LOWER(m.doc_name) LIKE LOWER(:search_text) LIMIT :page_size OFFSET ((:page_no - 1) * :page_size)`;
        ...
        for (let i = 0; row1 && i < row1.length; i++) {
            var map_entity = [];
            const _query2 = `SELECT m.entity_id, e.entity_name, m.is_required FROM csr_policy_doc_mapp m INNER JOIN entity_type e ON m.entity_id = e.entity_id WHERE m.document_id = ?`;
...
const csr_policy_docs_get = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query1 = `SELECT document_id, doc_name, doc_lng_key, file_type_allowed, file_max_size, is_enabled, added_date, modify_date FROM csr_policy_doc_mast WHERE document_id = ? AND is_deleted = false`;
...
const csr_policy_docs_new = async (req, res, next) => {
    ...
    try {
        ...
        const _query1 = `SELECT document_id FROM csr_policy_doc_mast WHERE LOWER(doc_name) = LOWER(?) AND is_deleted = false`;
        ...
        const _query2 = `INSERT INTO csr_policy_doc_mast(doc_name, doc_lng_key, file_type_allowed, file_max_size, is_enabled, is_deleted, added_by, added_date, sort_order)
        ...
        if (document_id > 0) {
            for (let k = 0; _map_entity && k < _map_entity.length; k++) {
                const _query3 = `INSERT INTO csr_policy_doc_mapp(document_id, entity_id, is_required) VALUES (?, ?, ?)`;
...
const csr_policy_docs_update = async (req, res, next) => {
    ...
    try {
        ...
        const _query0 = `SELECT document_id FROM csr_policy_doc_mast WHERE document_id = ? AND is_deleted = false`;
        ...
        const _query1 = `SELECT document_id FROM csr_policy_doc_mast WHERE document_id <> ? AND LOWER(doc_name) = LOWER(?) AND is_deleted = false`;
        ...
        const _query2 = `UPDATE csr_policy_doc_mast SET sort_order = ?, doc_name = ?, doc_lng_key = ?, file_type_allowed = ?, file_max_size = ?, modify_by = ?, modify_date = ? WHERE document_id = ?`;
        ...
        if (i > 0) {
            for (let k = 0; _map_entity && k < _map_entity.length; k++) {
                const _query3 = `INSERT INTO csr_policy_doc_mapp(document_id, entity_id, is_required) 
                SELECT :document_id, :entity_id, :is_required WHERE NOT EXISTS (
                    SELECT entity_id FROM csr_policy_doc_mapp WHERE document_id = :document_id AND entity_id = :entity_id
                )`;
                ...
                const _query5 = `UPDATE csr_policy_doc_mapp SET is_required = ? WHERE document_id = ? AND entity_id = ?`;
            ...
            const _query4 = `DELETE FROM csr_policy_doc_mapp WHERE document_id = ? AND entity_id NOT IN (?)`;
...
const csr_policy_docs_toggle = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query1 = `SELECT document_id, doc_name, is_enabled FROM csr_policy_doc_mast WHERE document_id = ? AND is_deleted = false`;
        const row1 = await db.sequelize.query(_query1, { replacements: [_document_id], type: QueryTypes.SELECT });
        if (!row1 || row1.length <= 0) {
            return res.status(200).json(success(false, apiStatus.RELOAD_PAGE,"Document details not found.", null));
        }

        const _query2 = `UPDATE csr_policy_doc_mast SET is_enabled = CASE WHEN is_enabled = true THEN false ELSE true END, modify_date = ?, modify_by = ? WHERE document_id = ?`;
...
const csr_policy_docs_delete = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query3 = `SELECT document_id FROM csr_policy_doc_mast WHERE document_id = ? AND is_deleted = false`;
        const row3 = await db.sequelize.query(_query3, { replacements: [_document_id], type: QueryTypes.SELECT });
        if (!row3 || row3.length <= 0) {
            return res.status(200).json(success(false, res.statusCode, "Document details not found.", null));
        }
        const _query2 = `UPDATE csr_policy_doc_mast SET is_deleted = true, deleted_date = ?, deleted_by = ? WHERE document_id = ?`;
        const _replacements2 = [new Date(), req.token_data.account_id, _document_id];
        const [, i] = await db.sequelize.query(_query2, { replacements: _replacements2, type: QueryTypes.UPDATE });
        if (i > 0) {
...
module.exports = {
    csr_policy_docs_list,
    csr_policy_docs_get,
    csr_policy_docs_new,
    csr_policy_docs_update,
    csr_policy_docs_toggle,
    csr_policy_docs_delete,
};