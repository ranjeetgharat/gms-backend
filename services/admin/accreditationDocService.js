...
const accreditation_doc_mast_list = async (req, res, next) => {
    ...
    try {
        ...
        const _query0 = `SELECT count(1) AS total_record FROM accreditation_doc_mast WHERE is_deleted = false AND LOWER(doc_name) LIKE LOWER(:search_text)`;
        ...
        FROM accreditation_doc_mast m WHERE m.is_deleted = false AND LOWER(m.doc_name) LIKE LOWER(:search_text) LIMIT :page_size OFFSET ((:page_no - 1) * :page_size)`;
        ...
        for (let i = 0; row1 && i < row1.length; i++) {
            var map_level = [];
            const _query2 = `SELECT m.level_id, e.level_name, m.is_required FROM accreditation_doc_mapp m INNER JOIN accreditation_level e ON m.level_id = e.level_id WHERE m.document_id = ? ORDER BY e.level_rank`;
...
const accreditation_doc_mast_get = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query1 = `SELECT document_id, doc_name, doc_lng_key, file_type_allowed, file_max_size, is_enabled, added_date, modify_date FROM accreditation_doc_mast WHERE document_id = ? AND is_deleted = false`;
...
const accreditation_doc_mast_new = async (req, res, next) => {
    const { doc_name, doc_lng_key, file_type_allowed, file_max_size, map_level, sort_order } = req.body;
    try {
        ...
        var _file_max_size = file_max_size && validator.isNumeric(file_max_size.toString()) ? BigInt(file_max_size) : 0;
        var _sort_order = sort_order && validator.isNumeric(sort_order.toString()) ? BigInt(sort_order) : 0;

        const _query1 = `SELECT document_id FROM accreditation_doc_mast WHERE LOWER(doc_name) = LOWER(?) AND is_deleted = false`;
        ...
        var _map_level = map_level;
        if (_map_level.length <= 0) {
            return res.status(200).json(success(false, res.statusCode, "Please select applicable level.", null));
        }

        const _query2 = `INSERT INTO accreditation_doc_mast(doc_name, doc_lng_key, file_type_allowed, file_max_size, added_by, added_date, sort_order)
        ...
        if (document_id > 0) {
            for (let k = 0; _map_level && k < _map_level.length; k++) {
                const _query3 = `INSERT INTO accreditation_doc_mapp(document_id, level_id, is_required) VALUES (?, ?, ?)`;
                const _replacements3 = [document_id, _map_level[k].level_id, _map_level[k].is_required];
...
const accreditation_doc_mast_update = async (req, res, next) => {
    const { document_id, doc_name, doc_lng_key, file_type_allowed, file_max_size, map_level, sort_order } = req.body;
    try {
        if (!doc_name || doc_name.length <= 0 || doc_name.trim().length <= 0) {
            return res.status(200).json(success(false, res.statusCode, "Please enter document name.", null));
        }
        ...
        const _query0 = `SELECT document_id FROM accreditation_doc_mast WHERE document_id = ? AND is_deleted = false`;
        ...
        const _query1 = `SELECT document_id FROM accreditation_doc_mast WHERE document_id <> ? AND LOWER(doc_name) = LOWER(?) AND is_deleted = false`;
        ...
        const _query2 = `UPDATE accreditation_doc_mast SET sort_order = ?, doc_name = ?, doc_lng_key = ?, file_type_allowed = ?, file_max_size = ?, modify_by = ?, modify_date = ? WHERE document_id = ?`;
        ...
        if (i > 0) {
            for (let k = 0; _map_level && k < _map_level.length; k++) {
                const _query3 = `INSERT INTO accreditation_doc_mapp(document_id, level_id, is_required) 
                SELECT :document_id, :level_id, :is_required WHERE NOT EXISTS (
                    SELECT level_id FROM accreditation_doc_mapp WHERE document_id = :document_id AND level_id = :level_id
                )`;
                await db.sequelize.query(_query3, {
                    replacements: {
                        document_id: _document_id, level_id: _map_level[k].level_id, is_required: _map_level[k].is_required
                    }, type: QueryTypes.INSERT
                });

                const _query5 = `UPDATE accreditation_doc_mapp SET is_required = ? WHERE document_id = ? AND level_id = ?`;
                await db.sequelize.query(_query5, { replacements: [_map_level[k].is_required, _document_id, _map_level[k].level_id], type: QueryTypes.UPDATE });
            }
            var temp = [];
            for (let k = 0; _map_level && k < _map_level.length; k++) {
                temp.push(_map_level[k].level_id);
            }
            const _query4 = `DELETE FROM accreditation_doc_mapp WHERE document_id = ? AND level_id NOT IN (?)`;
...
const accreditation_doc_mast_toggle = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query1 = `SELECT document_id, doc_name, is_enabled FROM accreditation_doc_mast WHERE document_id = ? AND is_deleted = false`;
        const row1 = await db.sequelize.query(_query1, { replacements: [_document_id], type: QueryTypes.SELECT });
        if (!row1 || row1.length <= 0) {
            return res.status(200).json(success(false, apiStatus.RELOAD_PAGE,"Document details not found.", null));
        }

        const _query2 = `UPDATE accreditation_doc_mast SET is_enabled = CASE WHEN is_enabled = true THEN false ELSE true END, modify_date = ?, modify_by = ? WHERE document_id = ?`;
...
const accreditation_doc_mast_delete = async (req, res, next) => {
    const { document_id } = req.body;
    try {
        var _document_id = document_id && validator.isNumeric(document_id.toString()) ? BigInt(document_id) : 0;

        const _query3 = `SELECT document_id FROM accreditation_doc_mast WHERE document_id = ? AND is_deleted = false`;
        const row3 = await db.sequelize.query(_query3, { replacements: [_document_id], type: QueryTypes.SELECT });
        if (!row3 || row3.length <= 0) {
            return res.status(200).json(success(false, res.statusCode, "Document details not found.", null));
        }
        const _query2 = `UPDATE accreditation_doc_mast SET is_deleted = true, deleted_date = ?, deleted_by = ? WHERE document_id = ?`;
        const _replacements2 = [new Date(), req.token_data.account_id, _document_id];
        const [, i] = await db.sequelize.query(_query2, { replacements: _replacements2, type: QueryTypes.UPDATE });
        if (i > 0) {
...
module.exports = {
    accreditation_doc_mast_list,
    accreditation_doc_mast_get,
    accreditation_doc_mast_new,
    accreditation_doc_mast_update,
    accreditation_doc_mast_toggle,
    accreditation_doc_mast_delete,
};